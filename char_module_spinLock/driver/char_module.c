...
static int mould_open(struct inode *inode, struct file *filp)
{
	...
	spin_lock_irqsave(&chardevbase.lock, flags); /*  上锁 */
	if (chardevbase.dev_stats) { /*  如果设备被使用了 */
		spin_unlock_irqrestore(&chardevbase.lock, flags); /*  解锁 */
		return -EBUSY;
	}
	chardevbase.dev_stats++; /*  如果设备没有打开，那么就标记已经打开了 */
	spin_unlock_irqrestore(&chardevbase.lock, flags);/*  解锁 */
...
{
	unsigned long flags;
	struct chardevbase_dev *dev = filp->private_data;
...
static int __init mould_init(void)
{
	...
	/* 初始化自旋锁 */
	spin_lock_init(&chardevbase.lock);
